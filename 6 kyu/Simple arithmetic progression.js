...
- Iterate from 1 to 'MAX_PROGRESSION' using index 'curProg'
  - Iterate through each number of the given array
    - For the current number, check if that number plus 'curProg' and number plus 'curProg' * 2 exists within the given array
      - If so, add the current number, the number plus 'curProg', and  the number plus 'curProg' * 2 to 'arraySet'
...
function solve(arr) {
  ...
  for (let curProg = 1; curProg <= MAX_PROGRESSION; curProg++) {
    for (let curNum of arr) {
      if (arr.includes(curNum) && arr.includes(curNum + curProg) && arr.includes(curNum + curProg * 2)) {
